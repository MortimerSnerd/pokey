...
proc newGLState*() : GLState {.raises: [GLError,ValueError,IOError].} = 
  ...
  result.solidColor = newProgram(result.rset, 
                                 [newShaderFromFile(result.rset, "color.vtx", GL_VERTEX_SHADER), 
                                  newShaderFromFile(result.rset, "color.frag", GL_FRAGMENT_SHADER)])
